...
| 入口網站頁面與品牌不搭 | 入口網站、登入和公開頁面均使用品牌色彩 |
...
- **內部用戶** — 在整個後台介面中看到自訂品牌色彩
- **入口網站用戶** — 入口網站頁面（帳戶首頁、報價單、發票、專案、任務）使用相同品牌色彩
- **公開用戶** — 登入、註冊和密碼重設頁面也套用品牌色彩
- **行動裝置用戶** — 響應式入口網站頁面和漢堡選單跟隨品牌色彩
...
│  │  ├── 所有用戶的前台 CSS（入口網站、公開、內部）             │   │
│  │  ├── 入口網站頁面、登入/註冊、導覽列、按鈕、連結           │   │
│  │  └── 行動裝置漢堡選單及編輯器啟動按鈕修復                   │   │
│  └────────────────────────────────────────────────────────────┘   │
│                              │                                    │
│  ┌──────────────────────────▼────────────────────────────────┐   │
...
├── __init__.py
├── __manifest__.py                     # 模組元資料 (v18.0.1.5.0)
├── controllers/
│   ├── __init__.py
...
2. 模板會為**所有用戶**注入 CSS — 內部、入口網站和公開用戶
3. 嘗試清除瀏覽器快取或強制重新整理（`Ctrl+Shift+R`）
...
- **功能：** 入口網站用戶現在看到與內部用戶相同的品牌色彩 — 統一品牌體驗
- **功能：** 登入、註冊和密碼重設頁面套用品牌色彩
- **功能：** 涵蓋入口網站頁面：帳戶首頁、報價單、發票、專案、任務、簽名
- **功能：** 行動裝置入口網站頁面套用品牌色彩（漢堡選單、響應式元素）
- **改善：** 新增 250+ 行入口網站/公開用戶 CSS 規則
